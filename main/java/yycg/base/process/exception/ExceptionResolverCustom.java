...
public class ExceptionResolverCustom implements HandlerExceptionResolver{
	...
	// json转换器，将异常信息转json 通过泛型 本系统的异常信息固定为ExceptionResultInfo
	private HttpMessageConverter<Exception> jsonMessageConverter;
	
	public HttpMessageConverter<Exception> getJsonMessageConverter() {
		return jsonMessageConverter;
	}
	public void setJsonMessageConverter(HttpMessageConverter<Exception> jsonMessageConverter) {
		this.jsonMessageConverter = jsonMessageConverter;
	}
	
	// 前端控制器调用此方法进行异常处理
	// handler，就是最终要执行的action类包装了一个方法（对应url的方法）
	@Override
	public ModelAndView resolveException(HttpServletRequest request,
		...
		modelAndView.addObject("exceptionResultInfo",
				exceptionResultInfo.getResultInfo());
		modelAndView.setViewName("WEB-INF/jsp/base/error");// 逻辑视图名
		return modelAndView;
	}
